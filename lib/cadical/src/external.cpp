...
// Taken from MaxHS
bool External::find_up_implicants(const std::vector<int>& antecedents, vector<int>& implicants) {
   reset_extended();
...
bool External::find_up_implicants(const std::set<int>& antecedents, vector<int>& implicants) {
    reset_extended();
    for (auto& elit : antecedents) assume(elit);
    update_molten_literals();
    auto res = internal->find_up_implicants(implicants);
