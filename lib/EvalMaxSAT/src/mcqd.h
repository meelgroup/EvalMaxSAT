...
class Maxclique {
  ...
  StepCount *S;
  bool connection(const int i, const int j) const { return e_conn[i][j]; }
  ...
  void expand_dyn(Vertices R, const int &maxStep) {
  ...
  void _mcq(int* &maxclique, int &sz, const int &maxStep, bool dyn) {
    V.set_degrees(*this);
    V.sort();
    V.init_colors();
    if (dyn) {
...
public:
  ...
  void mcq(int* &maxclique, int &sz, const int maxStep) { _mcq(maxclique, sz, maxStep, false); }
  void mcqdyn(int* &maxclique, int &sz, const int maxStep) { _mcq(maxclique, sz, maxStep, true); }
  ~Maxclique() {
    if (C) delete [] C;
    if (S) delete [] S;
    V.dispose();
