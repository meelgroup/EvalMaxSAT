...
void Internal::init_search_limits () {
  ...
  if(inc.timeout_seconds < 0) {
      lim.time_point_timeout = lim.time_point_timeout.max();
      LOG ("no timeout on conflicts");
  } else {
      lim.time_point_timeout = std::chrono::steady_clock::now() + std::chrono::duration_cast<std::chrono::nanoseconds>(std::chrono::duration<double>(inc.timeout_seconds));
 // std::chrono::milliseconds(inc.timeout);
      LOG ("set timout limit %" PRId64 "sec",
        lim.time_point_timeout);
  }
