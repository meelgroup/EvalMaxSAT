...
bool Solver::setTimeout(double seconds) {
    TRACE ("setTimeout", seconds);
    REQUIRE_VALID_STATE ();
    bool res = internal->setTimeout(seconds);
    LOG_API_CALL_END ("setTimeout", seconds);
    return res;
}
...
// Taken from MaxHS
bool Solver::find_up_implicants(const std::vector<int> &antecedents,
...
std::vector<bool> Solver::getSolution() {
    REQUIRE_VALID_STATE ();
    REQUIRE (state () == SATISFIED,
      "can only get value in satisfied state");
    if (!external->extended) external->extend ();
